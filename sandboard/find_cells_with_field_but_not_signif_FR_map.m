...
%% load cells FR maps
cells = cellfun(@(c)(cell_load_data(c,'details','fields','signif')), cells_t.cell_ID, 'UniformOutput',0);
cells = [cells{:}];

%% find cells with field but no significant FR map
list_cells_field_no_signif_FR_map = {};
for ii_cell = 1:length(cells)
    TF = 0;
    cell=cells(ii_cell);
    for ii_dir = 1:2
        fields = cells(ii_cell).fields{ii_dir};
        if ~cell.signif(ii_dir).TF & ~isempty(fields)
            TF = 1;
        end
    end
    if TF 
        list_cells_field_no_signif_FR_map{end+1} = cells_t.cell_ID{ii_cell};
    end
end

%% create folder with those cells figures
dir_IN = 'L:\Analysis\Results\cells\figures';
dir_OUT = 'L:\Analysis\Results\cells\with_field_but_no_signif_FR_map';
tmpl_list = list_cells_field_no_signif_FR_map;
ext = 'tif';
