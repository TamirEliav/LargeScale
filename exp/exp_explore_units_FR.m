function exp_explore_units_FR(exp_ID, NTT_file)
...
%% check input
if nargin==1
    [NTT_filename NTT_path] = uigetfile(fullfile(PLX_path,'*.NTT'), 'choose original ntt file (header copy)');
...
for ii_dir = 1:2
    ...
    for ii_cell = 1:length(cells)
        cell = cells(ii_cell);
        plot(cell.FR_map(ii_dir).bin_centers, cell.FR_map(ii_dir).PSTH,...
    ...
    plot( [exp_FE.pos], [exp_FE.ts], '.', 'Color', 0.9.*[1 1 1]);
    for ii_cell = 1:length(cells)
        cell_FE = cells(ii_cell).FE{ii_dir};
        plot([cell_FE.spikes_pos],[cell_FE.spikes_ts], '.', 'Color', units_colors(ii_cell,:));
    end
    ylim(ti)
