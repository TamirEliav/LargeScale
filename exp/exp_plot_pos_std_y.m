...
pos_y_std = exp.flight.pos_y_std;
...
for ii_dir = 1:2
    ...
    plot(pos_y_std(ii_dir).xy(:,1),...
         pos_y_std(ii_dir).xy(:,2),...
         '.', 'color', dir_colors{ii_dir},'MarkerSize',1);
    
    % plot the mean+std
    h=shadedErrorBar(pos_y_std(ii_dir).bin_centers,...
                     pos_y_std(ii_dir).ymean,...
                     pos_y_std(ii_dir).ystd,...
...
for ii_dir = 1:2
    plot(pos_y_std(ii_dir).bin_centers,...
         pos_y_std(ii_dir).ystd,...
         '-','color', dir_colors{ii_dir}, 'LineWidth',2);
     
     plot(pos_y_std(ii_dir).bin_centers,...
          diff(pos_y_std(ii_dir).yrange),...
         '--','color', dir_colors{ii_dir}, 'LineWidth',2);
end
columnlegend(2,{...
    sprintf('std %d%% trim',pos_y_std(ii_dir).trim_prc),...
    sprintf('range %d-%d%%',[pos_y_std(ii_dir).range_prc(:)]),...
    sprintf('std %d%% trim',pos_y_std(ii_dir).trim_prc),...
    sprintf('range %d-%d%%',[pos_y_std(ii_dir).range_prc(:)]),...
     } );
xlabel('X Position (m)')
...
for ii_dir = 1:2
    dev_std = pos_y_std(ii_dir).ystd;
    dev_range = diff(pos_y_std(ii_dir).yrange);
