...
while hole_i <= length(holes_ind_start)
    ...
    if holes_size_t(hole_i)<=(interpolation_always_TH) %small hole - ALWAYS LINEAR INTERPOLATION
        ...
        fill_in_pos = coeffs.p1*fill_in_ts + coeffs.p2;
    ...
    if holes_size_t(hole_i) < large_hole_TH  %interpolation
        ...
        side1_ind = find(abs(raw_data_ts_usec(holes_ind_start(hole_i))-raw_data_ts_usec(1:holes_ind_end(hole_i)-1))<=time_for_fitting_extra);
        side2_ind = holes_ind_start(hole_i) + find(abs(raw_data_ts_usec(holes_ind_end(hole_i))-raw_data_ts_usec(holes_ind_start(hole_i)+1:end))<=time_for_fitting_extra);
        ...
        fill_in_pos_side1 = coeffs_side1.p1*fill_in_ts_side1 + coeffs_side1.p2;
        
        fill_in_ts_side2 = fill_in_ts((end-samples_to_fill_eachside_extrapolation+1):end);
        fill_in_pos_side2 = coeffs_side2.p1*fill_in_ts_side2 + coeffs_side2.p2;
