...
%% arrange events to plot
events = struct();
PE_ti = [exp.PE.thr.start_ts;exp.PE.thr.end_ts]';
rest_ti = exp.rest.ti;
sdf = any(PE_ti>shiftdim(rest_ti(:,1),-2) & PE_ti<shiftdim(rest_ti(:,2),-2), [2 3]);


%%
for ii_rest = 1:length(exp.rest.events)
    ...
    fig_name = sprintf('%s_rest_%d', exp_ID, ii_rest);
    ...
    % arrange data
    rest = exp.rest.events(ii_rest);
    ti = [rest.start_ts rest.end_ts];
    t0 = ti(1);
