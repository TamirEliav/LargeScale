...
[pval12_rank,~,rank_stat_12] = ranksum(X1,X2,'tail','right');
% pval13_rank = ranksum(X1,X3,'tail','right');
fprintf('Field num 1 vs. 2: ttest p=%.4g tstat=%.2f df=%d, ranksum p=%.4g zval=%.4g\n',pval12, tstat12.tstat, tstat12.df, pval12_rank,rank_stat_12.zval);
...
[pval12_rank,~,rank_stat_12] = ranksum(X1,X2,'tail','right');
% pval13_rank = ranksum(X1,X3,'tail','right');
fprintf('Field size 1 vs. 2: ttest p=%.4g tstat=%.2f df=%d, ranksum p=%.4g zval=%.4g\n',pval12, tstat12.tstat, tstat12.df, pval12_rank,rank_stat_12.zval);
...
[pval12_rank,~,rank_stat_12] = ranksum(X1,X2,'tail','right');
% pval13_rank = ranksum(X1,X3,'tail','right');
fprintf('L/S ratio 1 vs. 2: ttest p=%.4g tstat=%.2f df=%d, ranksum p=%.4g zval=%.4g\n',pval12, tstat12.tstat, tstat12.df, pval12_rank,rank_stat_12.zval);
