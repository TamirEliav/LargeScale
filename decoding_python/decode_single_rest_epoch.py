...
def main():
    ...
    parser = argparse.ArgumentParser(description='decode during a specific rest on the balls')
    ...
    args.rest_idx = args.job_index-1
    ...
    exp.rest_predict(rest_idx = args.rest_idx)
    end = time.time()
    print('analysis time:',end-start,'s')
    print('saving results')
    exp.rest_results.attrs.update({'exp_ID' : args.exp_ID,
                                   'rest_idx' : args.rest_idx,
                                   'pos_bin_size' : args.pos_bin_size,
                                   'pos_std' : args.pos_std,
                                   'mark_std' : args.mark_std,
    ...
    exp.save_rest_res(folder=folder)
    print('results saved')
