...
prm.flight.speed_high_thr = 2;
prm.flight.speed_low_thr = 0.75;
prm.flight.high_speed_min_duration = 0.5;
prm.flight.full_min_distance = 3;
...
prm.FR_map.bin_size = 0.1; %0.2;
prm.FR_map.bin_limits = [0 200];
prm.FR_map.ker_SD = 0.2; %0.40 0.50 %0.5;
...
prm.fields.valid_speed_pos = [0.3 -0.3]; %Shir changed - now it is represented 
% as distance from tha balls and not as absolute location 
% [ball1 ball2] + [x -x]
...
prm.signif.max_coverage = 80; % added by Shir - a condition on maximum 
% possible coverage for 6m maps.

prm.inclusion.min_full_flights = 10; % >=
prm.inclusion.min_spikes_air   = 25; % >= changed by Shir, original = 50
...
switch paramset_global_var
    ...
    case 'binning_option1' %added by shir
        prm.FR_map.bin_size = 0.1; 
        prm.FR_map.ker_SD = 0.2; 
    case 'binning_option2' %added by shir
        prm.FR_map.bin_size = 0.1; 
        prm.FR_map.ker_SD = 0.25; 
    case 'binning_option3' %added by shir
        prm.FR_map.bin_size = 0.2;
        prm.FR_map.ker_SD = 0.3; 
    case 'margin_option1' %added by shir
        prm.fields.valid_speed_pos = [0.2 -0.2];
    case 'margin_option2' %added by shir
        prm.fields.valid_speed_pos = [0.25 -0.25]; 
    case 'margin_option3' %added by shir
        prm.fields.valid_speed_pos = [0.3 -0.3];
end
